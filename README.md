...
local smooth = imgui.new.float(config.main.smooth)
local radius = imgui.new.float(config.main.radius)
local enable = imgui.new.bool(config.main.enable)
local clistFilter = imgui.new.bool(config.main.clistFilter)
local visibleCheck = imgui.new.bool(config.main.visibleCheck)
local checkStuned = imgui.new.bool(config.main.checkStuned)
local checkPause = imgui.new.bool(config.main.checkPause)
local idFilter = imgui.new.bool(config.main.idFilter)
local excludeIDs = config.main.excludeIDs
local excludeIDsBuffer = imgui.new.char[256](config.main.excludeIDs)
local heightOffset = imgui.new.float(config.main.heightOffset)
local useHeightOffset = imgui.new.bool(config.main.useHeightOffset)
local fovAdjust = imgui.new.float(config.main.fovAdjust)
local disableOnZoom = imgui.new.bool(config.main.disableOnZoom)
local zOffset = imgui.new.float(config.main.zOffset)
local useZOffset = imgui.new.bool(config.main.useZOffset)
...
function GetAimbotBodyPartCoords(id, handle)
    ...
    if res then
        if useHeightOffset[0] then
            return vec.x, vec.y, vec.z + heightOffset[0]
        else
        ...
        if handle ~= -1 then
            ...
            if result then
                ...
                if not visibleCheck[0] or (visibleCheck[0] and isLineOfSightClear(myX, myY, myZ, enX, enY, enZ, true, true, false, true, true)) then
                    ...
                    local angle = {
                        (math.atan2(vector[2], vector[1]) + 0.04253), 
                        (math.atan2((math.sqrt((math.pow(vector[1], 2) + math.pow(vector[2], 2)))), vector[3]) - math.pi / 2 - coefficentZ + (useZOffset[0] and zOffset[0] or 0.0))
                    }
                    local view = {fix(representIntAsFloat(readMemory(0xB6F258, 4, false))), fix(representIntAsFloat(readMemory(0xB6F248, 4, false)))}
                    local difference = {angle[1] - view[1], angle[2] - view[2]}
                    local smoothFactor = smooth[0]
                    if isAiming and (weaponId == 30 or weaponId == 31) then
...
imgui.OnFrame(function() return mainWindow[0] end, function(player)
    ...
    if imgui.Begin('Smooth Aimbot v2.0', mainWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse) then
        ...
        if imgui.SliderFloat('##Radius', radius, 0.0, 100.0, 'Radius: %.1f') then config.main.radius = radius[0] end
        if imgui.SliderFloat('##Smooth', smooth, 0.0, 50.0, 'Smooth: %.1f') then config.main.smooth = smooth[0] end
        if imgui.SliderFloat('##HeightOffset', heightOffset, -1.0, 1.0, 'Height Offset: %.2f') then config.main.heightOffset = heightOffset[0] end
        imgui.SameLine()
        if imgui.Checkbox('Use Height Offset', useHeightOffset) then config.main.useHeightOffset = useHeightOffset[0] end
        if imgui.SliderFloat('##FOVAdjust', fovAdjust, -500.0, 500.0, 'FOV Adjust: %.1f') then config.main.fovAdjust = fovAdjust[0] end
        if imgui.SliderFloat('##ZOffset', zOffset, -1.0, 1.0, 'Z Offset: %.2f') then config.main.zOffset = zOffset[0] end
        imgui.SameLine()
        if imgui.Checkbox('Use Z Offset', useZOffset) then config.main.useZOffset = useZOffset[0] end
        imgui.PopStyleVar()

        imgui.Columns(2, nil, false)
        imgui.PushStyleVarVec2(imgui.StyleVar.ItemSpacing, imgui.ImVec2(8, 8))
        if imgui.Checkbox('Enable', enable) then config.main.enable = enable[0] end
        imgui.NextColumn()
        if imgui.Checkbox('Visible Check', visibleCheck) then config.main.visibleCheck = visibleCheck[0] end
        imgui.NextColumn()
        if imgui.Checkbox('Check Stuned', checkStuned) then config.main.checkStuned = checkStuned[0] end
        imgui.NextColumn()
        if imgui.Checkbox('Clist Filter', clistFilter) then config.main.clistFilter = clistFilter[0] end
        imgui.NextColumn()
        if imgui.Checkbox('Check Pause', checkPause) then config.main.checkPause = checkPause[0] end
        imgui.NextColumn()
        if imgui.Checkbox('ID Filter', idFilter) then config.main.idFilter = idFilter[0] end
        imgui.NextColumn()
        if imgui.Checkbox('Disable on Zoom', disableOnZoom) then config.main.disableOnZoom = disableOnZoom[0] end
        imgui.Columns(1)
        imgui.PopStyleVar()

        imgui.Text('Exclude IDs (comma separated):')
        if imgui.InputText('##ExcludeIDs', excludeIDsBuffer, 256) then
            config.main.excludeIDs = ffi.string(excludeIDsBuffer)
        end
        imgui.Dummy(imgui.ImVec2(0, 10))

        inicfg.save(config, "smooth_aimbot")
        imgui.End()
    end
end)
...
function main()
    ...
    -- Load Aimbot Config
    local configFile = inicfg.load(config, "smooth_aimbot")
    if configFile then
        smooth[0] = configFile.main.smooth or 0.0
        radius[0] = configFile.main.radius or 0.6
        enable[0] = configFile.main.enable or false
        clistFilter[0] = configFile.main.clistFilter or false
        visibleCheck[0] = configFile.main.visibleCheck or false
        checkStuned[0] = configFile.main.checkStuned or false
        checkPause[0] = configFile.main.checkPause or false
        idFilter[0] = configFile.main.idFilter or false
        excludeIDs = tostring(configFile.main.excludeIDs or "-1")
        heightOffset[0] = configFile.main.heightOffset or 0.0
        useHeightOffset[0] = configFile.main.useHeightOffset ~= nil and configFile.main.useHeightOffset or true
        fovAdjust[0] = configFile.main.fovAdjust or 70.0
        disableOnZoom[0] = configFile.main.disableOnZoom or false
        zOffset[0] = configFile.main.zOffset or 0.0
        useZOffset[0] = configFile.main.useZOffset ~= nil and configFile.main.useZOffset or true
        excludeIDsBuffer = imgui.new.char[256](excludeIDs)
    end
    ...
    -- Чат-команда /traisers теперь открывает меню настроек аимбота!
